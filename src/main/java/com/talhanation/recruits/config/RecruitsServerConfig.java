...
public class RecruitsServerConfig {
    ...
    public static ForgeConfigSpec.BooleanValue EnableOptionalFlowFieldPrototype;
    public static ForgeConfigSpec.IntValue FlowFieldPrototypeMinCohortSize;
    ...
    public static ForgeConfigSpec.BooleanValue EnableAiLod;
    public static ForgeConfigSpec.IntValue AiLodFullDistance;
    public static ForgeConfigSpec.IntValue AiLodReducedDistance;
    public static ForgeConfigSpec.IntValue AiLodReducedTargetSearchInterval;
    public static ForgeConfigSpec.IntValue AiLodShedTargetSearchInterval;
    ...
    static {
        ...
        */
        ...
        EnableOptionalFlowFieldPrototype = BUILDER.comment("""
                        Enable the optional flow-field navigation prototype for large same-destination recruit groups.
                        This remains a guarded experiment and falls back to normal pathfinding when a request is out of scope.
                        \t(takes effect after restart)
                        \tdefault: false""")
                .worldRestart()
                .define("EnableOptionalFlowFieldPrototype", false);

        FlowFieldPrototypeMinCohortSize = BUILDER.comment("""
                        Minimum number of nearby same-group recruits moving toward the same static destination before the optional flow-field prototype is eligible.
                        Smaller groups stay on the normal navigation stack.
                        \t(takes effect after restart)
                        \tdefault: 6""")
                .worldRestart()
                .defineInRange("FlowFieldPrototypeMinCohortSize", 6, 2, 64);
        ...
        EnableAiLod = BUILDER.comment("""
                        Enable explicit AI LOD for recruit target-search cadence.
                        Close or recently-engaged recruits stay full fidelity, while lower-priority recruits may search less often.
                        \t(takes effect after restart)
                        \tdefault: true""")
                .worldRestart()
                .define("EnableAiLod", true);

        AiLodFullDistance = BUILDER.comment("""
                        Distance in blocks where recruit target-search stays full fidelity.
                        Nearby players or nearby live targets inside this range keep the original 20-tick cadence.
                        \t(takes effect after restart)
                        \tdefault: 16""")
                .worldRestart()
                .defineInRange("AiLodFullDistance", 16, 0, 256);

        AiLodReducedDistance = BUILDER.comment("""
                        Distance in blocks where recruit target-search falls back to the reduced AI LOD tier.
                        Recruits beyond this range with no active nearby combat may use the shed tier.
                        \t(takes effect after restart)
                        \tdefault: 40""")
                .worldRestart()
                .defineInRange("AiLodReducedDistance", 40, 0, 256);

        AiLodReducedTargetSearchInterval = BUILDER.comment("""
                        Target-search interval in ticks for the reduced AI LOD tier.
                        Must stay aligned to the original 20-tick stagger.
                        \t(takes effect after restart)
                        \tdefault: 40""")
                .worldRestart()
                .defineInRange("AiLodReducedTargetSearchInterval", 40, 20, 400);

        AiLodShedTargetSearchInterval = BUILDER.comment("""
                        Target-search interval in ticks for the shed AI LOD tier.
                        This only affects non-critical target-search work and must stay aligned to the original 20-tick stagger.
                        \t(takes effect after restart)
                        \tdefault: 80""")
                .worldRestart()
                .defineInRange("AiLodShedTargetSearchInterval", 80, 20, 400);
