...
public final class CitizenBirthRules {
    ...
    public static Decision evaluate(BannerModSettlementBinding.Status status,
        ...
        if (claimFoodUnits < config.minFoodUnits()) {
            return deny(DenialReason.NO_FOOD, requiredCooldownTicks);
        }
     ...
     * @param minFoodUnits minimum aggregate food-item count across the claim's storage areas required to permit a birth
     */
    public record Config(boolean enabled,
        ...
        public Config {
            ...
            minFoodUnits = Math.max(0, minFoodUnits);
        }
    }
