...
import com.talhanation.bannerlord.client.shared.gui.RecruitsScreenBase;
...
public abstract class WorkAreaScreen extends RecruitsScreenBase {
    ...
    private static final ResourceLocation TEXTURE = WorkersRuntime.mergedGuiTexture("workareascreen.png");
    private static final MutableComponent TEXT_FORWARD = Component.translatable("gui.bannermod.workers.command.text.forward");
    private static final MutableComponent TEXT_BACKWARD = Component.translatable("gui.bannermod.workers.command.text.back");
    private static final MutableComponent TEXT_LEFT = Component.translatable("gui.bannermod.workers.command.text.left");
    private static final MutableComponent TEXT_RIGHT = Component.translatable("gui.bannermod.workers.command.text.right");
    private static final MutableComponent TEXT_DESTROY = Component.translatable("gui.bannermod.workers.command.text.destroy");
    ...
    public void setButtons(){
        ...
        moveForward = addRenderableWidget(new ExtendedButton(x - buttonWidth / 2, y + buttonHeight / 2 - buttonHeight*2, buttonWidth, buttonHeight, TEXT_FORWARD,
            btn -> {
                ...
                WorkersRuntime.channel().sendToServer(new MessageUpdateWorkArea(this.workArea.getUUID(), this.workArea.getCustomName().getString(), newPos, false));
                this.onAreaMoved();
            }
        ));
        ...
        destroy = addRenderableWidget(new ExtendedButton(x - buttonWidth / 2, y - buttonHeight / 2, buttonWidth, buttonHeight, TEXT_DESTROY,
            btn -> {
                WorkersRuntime.channel().sendToServer(new MessageUpdateWorkArea(this.workArea.getUUID(), this.workArea.getCustomName().getString(), workArea.position(), true));
                this.onClose();
            }
        ));
        ...
        rotateLeft = addRenderableWidget(new ExtendedButton(x - buttonWidth / 2 - buttonWidth, y - buttonHeight / 2 + buttonHeight, buttonWidth, buttonHeight, Component.literal("\u21BB"),
            btn -> {
                this.workArea.showBox = true;
                WorkersRuntime.channel().sendToServer(new MessageRotateWorkArea(this.workArea.getUUID(), false));
                this.onAreaMoved();
            }
        ));
        ...
        rotateRight = addRenderableWidget(new ExtendedButton(x - buttonWidth / 2 + buttonWidth, y - buttonHeight / 2 + buttonHeight, buttonWidth, buttonHeight, Component.literal("\u21BA"),
            btn -> {
                this.workArea.showBox = true;
                WorkersRuntime.channel().sendToServer(new MessageRotateWorkArea(this.workArea.getUUID(), true));
                this.onAreaMoved();
            }
        ));
        //OWNER STUFF
                ...
                button -> {
                    minecraft.setScreen(new SelectPlayerScreen(this, player, SelectPlayerScreen.TITLE, SelectPlayerScreen.BUTTON_SELECT, SelectPlayerScreen.BUTTON_SELECT_TOOLTIP, false, PlayersList.FilterType.NONE,
                            (playerInfo) -> {
                                ...
                                WorkersRuntime.channel().sendToServer(new MessageUpdateOwner(this.workArea.getUUID(), playerInfo));
