...
public class MerchantTradeScreen extends ScreenBase<MerchantTradeContainer> {
    ...
    private static final ResourceLocation RESOURCE_LOCATION = WorkersRuntime.mergedGuiTexture("merchant.png");
    private static final ResourceLocation ARROW_IMAGE = WorkersRuntime.mergedGuiTexture("arrow.png");
    private static final MutableComponent BUTTON_ADD = Component.translatable("gui.bannermod.workers.button.add");
    private static final MutableComponent BUTTON_EDIT = Component.translatable("gui.bannermod.workers.button.edit");
    private static final MutableComponent BUTTON_REMOVE = Component.translatable("gui.bannermod.workers.button.remove");
    private static final MutableComponent BUTTON_COPY = Component.translatable("gui.bannermod.workers.button.copy");
    private static final MutableComponent BUTTON_TRADE = Component.translatable("gui.bannermod.workers.button.trade");
    private static final MutableComponent TEXT_CREATIVE = Component.translatable("gui.bannermod.workers.text.creative");
    private static final MutableComponent TEXT_DAILY_REFRESH = Component.translatable("gui.bannermod.workers.text.dailyRefresh");
    ...
    public void setWidgets(){
        ...
        if((merchantEntity.isCreative() && player.isCreative()) || isOwner){
            tradeButton = new ExtendedButton(leftPos + 88, topPos + 58, 60, 18, BUTTON_TRADE,
                    button -> {
                        WorkersRuntime.channel().sendToServer(new MessageDoTradeWithMerchant(merchantEntity.getUUID(), selection.uuid));
            ...
            copyTradeButton = new ExtendedButton(leftPos + 88, topPos + 77, 60, 18, BUTTON_COPY,
                    button -> {
                        ...
                        WorkersRuntime.channel().sendToServer(new MessageUpdateMerchantTrade(this.merchantEntity.getUUID(), trade, false));
                        tradeList.addEntry(this.tradeList.new TradeEntry(trade));
                        this.selection = null;
                        tradeList.setSelected(null);
                        updateButtonState();
            ...
            removeTradeButton = new ExtendedButton(leftPos + 186, topPos + 77, 60, 18, BUTTON_REMOVE,
                    button -> {
                        ...
                        WorkersRuntime.channel().sendToServer(new MessageUpdateMerchantTrade(merchantEntity.getUUID(), selection, true));
                        this.selection = null;
                        tradeList.setSelected(null);
                        updateButtonState();
                    });
            ...
            moveUpButton = new ExtendedButton(leftPos + 158, topPos + 58, 18, 18, Component.literal("\u21E7"),//do not replace
                    button -> {
                        ...
                        WorkersRuntime.channel().sendToServer(new MessageMoveMerchantTrade(merchantEntity.getUUID(), selectedUuid, true));
            ...
            moveDownButton = new ExtendedButton(leftPos + 158, topPos + 77, 18, 18, Component.literal("\u21E9"),//do not replace
                    button -> {
                        if (selection == null) return;
                        UUID selectedUuid = selection.uuid;
                        WorkersRuntime.channel().sendToServer(new MessageMoveMerchantTrade(merchantEntity.getUUID(), selectedUuid, false));
                        List<WorkersMerchantTrade> list = new java.util.ArrayList<>(merchantEntity.getTrades());
            ...
            if(player.isCreative()) {
                this.creativeCheckbox = new RecruitsCheckBox(leftPos + 256, topPos + 172, 100, 20, TEXT_CREATIVE,
                        ...
                        (bool) -> {
                            ...
                            WorkersRuntime.channel().sendToServer(new MessageUpdateMerchant(merchantEntity.getUUID(), isCreative, true, isDailyRefresh));
    ...
    public void onClose() {
        super.onClose();
        WorkersRuntime.channel().sendToServer(new MessageUpdateMerchant(merchantEntity.getUUID(), isCreative, false, isDailyRefresh));
    }
