...
public final class RecruitsCommandGameTestSupport {
    ...
    public static CommandScenario spawnCommandScenario(GameTestHelper helper) {
        ...
        AbstractRecruitEntity otherGroupRecruit = RecruitsBattleGameTestSupport.spawnConfiguredRecruit(helper, com.talhanation.recruits.init.ModEntityTypes.RECRUIT.get(), OTHER_GROUP_POS, "Other Group", player.getUUID());
        prepareForCommand(otherGroupRecruit, OTHER_GROUP_UUID);

        AbstractRecruitEntity foreignRecruit = RecruitsBattleGameTestSupport.spawnConfiguredRecruit(helper, com.talhanation.recruits.init.ModEntityTypes.RECRUIT.get(), FOREIGN_RECRUIT_POS, "Foreign Recruit", FOREIGN_OWNER_UUID);
        prepareForCommand(foreignRecruit, TARGET_GROUP_UUID);

        AbstractRecruitEntity farRecruit = RecruitsBattleGameTestSupport.spawnConfiguredRecruit(helper, com.talhanation.recruits.init.ModEntityTypes.RECRUIT.get(), FAR_RECRUIT_POS, "Far Recruit", player.getUUID());
        prepareForCommand(farRecruit, TARGET_GROUP_UUID);

        AbstractRecruitEntity enemyRecruit = RecruitsBattleGameTestSupport.spawnConfiguredRecruit(helper, com.talhanation.recruits.init.ModEntityTypes.RECRUIT.get(), RecruitsBattleGameTestSupport.EAST_FRONTLINE_POS, "Enemy Recruit", FOREIGN_OWNER_UUID);
