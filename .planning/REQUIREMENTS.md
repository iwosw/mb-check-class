...
## Completed Stabilization Requirements
...
## Completed Dedicated-Server Authority Validation Requirements
...
- [x] DSAUTH-02 — Dedicated-server validation proves reconnect and persistence-safe ownership recovery stays deterministic across recruit, worker, and work-area state.

## Completed Phase 22 Requirements
...
## Completed Phase 21 Requirements
...
## Completed Phase 23 Requirements
...
- [x] GOV-04 — Governor control is validated through focused GameTests and player-facing UI flows over the real server-side governance snapshot.

## Consolidated Phase 30 Requirements

- [x] WBSP-01 — Worker birth and settlement spawn eligibility are evaluated from one explicit claim-aware rules seam instead of scattered event logic.
- [x] WBSP-02 — Runtime worker birth and autonomous settlement spawn remain friendly-claim only, bounded by explicit quota/cooldown rules, and deny hostile or unclaimed territory.
- [x] WBSP-03 — Root GameTests validate friendly worker birth, cooldown-bounded claim spawn, and hostile or unclaimed denial through the live runtime seam.
- [x] MINING-01 — Miners can excavate a diagonal tunnel pattern that moves away from the worker instead of digging directly under themselves.
- [x] MINING-02 — Miners support deterministic branch/strip mining with explicit tunnel/branch settings instead of a generic scan-box authoring contract.
- [x] MINING-03 — Mining progression preserves segment advancement while hostile-claim targets are skipped instead of stalling the miner against foreign territory.
- [x] BUILD-01 — Build areas can load schematic templates into the existing preview/build pipeline without replacing the current NBT-based workflow.

## Completed Phase 31 Requirements
...
## Compact Active Program (Phases 24-28)
...
- Compact Phase 28 owns architecture integration, telemetry, balance, migration, and safe-rollout closure that were previously split across old Phases 28, 39, and 49.
- Compact-phase intent note: the folded settlement track is a staged replacement of vanilla village dependencies, not a permanent plan to keep BannerMod settlement simulation layered on top of vanilla village logic forever.
- Historical requirement IDs above remain valid evidence for completed phases only. New requirement IDs for compact Phases 24-28 should be introduced when those compact phases are actually re-planned, rather than copied forward from the stale older numbering.
