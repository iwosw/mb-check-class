...
status: Compact Phase 26 combat AI plus its follow-up stance-control slices are landed — round-robin target spread, 60-tick recent-loss LOD boost, CombatStance (LOOSE/LINE_HOLD/SHIELD_WALL) with synced state + NBT persistence, formation-aware leash, gap-fill on neighbour death, slow body-yaw in formation, directional 120° shield block with stance-tuned mitigation via Forge ShieldBlockEvent cancellation, SHIELD_WALL auto-block + slow facing pivot, per-item weapon reach (sarissa/pike/spear plus common polearm/lance aliases and Better Combat attackRange metadata), native reflection-gated Better Combat attack metadata integration with BannerMod-owned upswing, damage callback, AOE arc hits, and synced recruit pose presentation, second-rank poke via FriendlyLineOfSight, per-unit attack cadence, flank (×1.15 side / ×1.5 back), formation cohesion (−15%), brace-for-cavalry with knockback resistance, HYW-parity unit-type counter matrix, player-facing command-screen stance wiring, per-recruit stance control in RecruitInventoryScreen, and ranged-goal stance leash. Compact Phase 25 work-order runtime remains the prior landed surface.
...
- Current focus: compact Phase 26 combat AI plus its player-facing stance-control follow-ups are the newest landed surface (slice status: `.planning/phases/26-army-command-formations-warfare/26-COMBAT-AI-SLICE-STATUS.md`). Target selection, line cohesion, directional shield block, reach weapons, rank-2 poke, per-unit cadence, flank/cohesion/brace, HYW unit-type counters, command-screen stance selection, server-authoritative stance packet routing, per-recruit inventory stance control, synced stance state, ranged-goal stance leash, common polearm/lance id aliases for reach/cadence, Better Combat `attackRange()` read path, and native Better Combat `attacks()` metadata handling through BannerMod-owned upswing/damage/AOE/presentation are all live. Better Mob Combat is intentionally not a dependency or adapter. Latest verification on 2026-04-25 kept `compileJava processResources` and targeted `BetterCombatAttackBridgeTest`/`BetterCombatWeaponAttributesTest`/`WeaponReachTest`/`AttackCadenceTest` green; `verifyGameTestStage` with Better Combat present started the GameTest server and loaded Better Combat but failed on existing `reconnectedownerrecoversauthorityafterownershiproundtrip`. Compact Phase 25 settlement work-order runtime remains the prior landed surface and stays available for follow-up slices.
...
- Active compact-Phase-26 planning artifacts: `.planning/phases/26-army-command-formations-warfare/` (combat AI overhaul landed 2026-04-21; the 2026-04-22 follow-up added stance command/UI wiring plus ranged-goal leash)
...
- [Post-Phase-06 roadmap]: Validate faction-binding enforcement separately from contract-definition so each later phase slice stays small and reviewable.
...
- [Compact Phase 26 combat AI]: Player-facing stance commands now route through `MessageCombatStance` and `CommandIntent.CombatStanceChange`, and the same follow-up extends `CombatLeashPolicy` into both ranged attack goals.
...
- [Phase 21-source-tree-consolidation-into-bannerlord]: workerPacketOffset() now returns MILITARY_MESSAGES.length (compile-time=106), making must-have #4 from 21-03 provable
