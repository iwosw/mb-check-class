...
4. Place a `Starter Fort` — the keystone building, no settlement spawns without one. Build it manually or use a prefab, then use the `Settlement Surveyor Tool`: right-click in the air to open the survey board, choose mode/role there, right-click the anchor block, then right-click corners of each required zone in the world. Shift+right-click on a block still cycles the zone role as a fallback, but normal mode/role selection now lives in the GUI. The starter fort must include `AUTHORITY_POINT` and `INTERIOR`; the anchor should be inside the authority area. The item tooltip and survey board both mirror this rule and the next settlement steps. When the session is filled, use the board's `Validate` button.
