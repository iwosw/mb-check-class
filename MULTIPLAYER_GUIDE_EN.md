...
Citizen-on-citizen birth runs as a separate optional pass (off by default — enable via `CitizenBirthEnabled`). When active, each claim is scanned every `CitizenBirthCooldownTicks` (default ~1 in-game day) for an opposite-gender adult pair; if there is at least one pair, the claim has free housing capacity, and the baby cap (`CitizenBirthMaxBabiesPerClaim`) is not yet reached, the server spawns a baby citizen at the mother's position. The baby becomes an adult after `CitizenBirthGrowUpTicks` (default ~7 days), after which auto-staffing can assign it a profession through the normal flow. Citizen gender is randomized at first spawn and persisted in NBT.
