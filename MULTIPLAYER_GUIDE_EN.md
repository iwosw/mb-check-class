...
5. After a successful starter-fort validation the settlement bootstraps automatically: a `SettlementRecord` is created, a starter claim is added if needed, and starter workers spawn near the anchor (farmer, miner, lumberjack, builder). The farmer is ready immediately because bootstrap seeds a starter crop area. The miner, lumberjack, and builder are intentionally waiting until you create/register a mine, lumber camp, and architect workshop/build area; the bootstrap message lists those next actions. Free citizens are a separate population source, not the same thing as these starter workers.
