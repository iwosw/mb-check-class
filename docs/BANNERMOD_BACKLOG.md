...
**Status: DONE 2026-04-27.** All four named subsystems — work-order, seller-phase, household-assignment, project-queue — have focused persistence tests covering NBT roundtrip + no-op restore + forward-compat enum fallback. The acceptance bullets ("Reload does not lose active meaningful settlement work state", "No false dirty churn on identical reload/restore", "Focused persistence tests pass") are met across the full named scope.
...
**Progress 2026-04-27.** Seller-phase persistence audited and locked down by `BannerModSellerDispatchPersistenceTest` (8 cases against `BannerModSellerDispatchRuntime.toTag` / `fromTag` and `SellerPhaseRecord.toTag` / `fromTag`): empty roundtrip preserves emptiness, mixed multi-seller mid-flight + terminal phases roundtrip record-for-record, `LinkedHashMap` insertion-order is preserved end-to-end (advisor iteration contract), `phaseTickCount` and `phaseStartGameTime` survive intact (no budget restart on reload), every `SellerPhase` enum value roundtrips to itself rather than silently falling back to READY, an unknown future phase name does fall back to READY without crashing the loader (forward-compat contract), tick-budget continues from the loaded `phaseTickCount` so a seller mid-MOVING_TO_STALL on save still trips into AT_STALL at the original budget boundary on load, and the decoded runtime is independent of the source (mutating one does not bleed into the other). The `noOpAdvanceAndIdenticalRestoreDoNotDirty` case in the existing `BannerModSellerDispatchRuntimeTest` already covered the no-op-restore-doesn't-dirty leg of the acceptance, so seller-phase is now fully audited. Acceptance bullets "Reload does not lose active meaningful settlement work state" and "No false dirty churn on identical reload/restore" and "Focused persistence tests pass" are met **for the seller-phase subsystem only**. Household-assignment and project-queue persistence still need the same audit pass before the slice can close.

**Progress 2026-04-27.** Household-assignment persistence audited. The existing `BannerModHomeAssignmentRuntimeTest` already covered basic NBT roundtrip plus the dirty-listener contract (mutations dirty / no-op assign + identical restore don't); the SETTLEMENT-004 audit gap was forward-compat coverage. New `BannerModHomeAssignmentPersistenceTest` (6 cases) supplements with: empty runtime roundtrips to empty (no spurious decoded entries), every `HomePreference` value roundtrips to itself rather than the NONE fallback (defends against accidental enum churn), an unknown future preference name does fall back to NONE without crashing the loader (forward-compat contract), three-resident non-alphabetical insertion order survives the roundtrip (LinkedHashMap iteration contract — a HashMap on the decode side would visibly fail), `assignedAtGameTime` survives a value beyond `Integer.MAX_VALUE` so a regression that narrowed the field to int would surface, and the decoded runtime is mutable independently of the source (no shared references after `fromTag`). Acceptance bullets are met **for seller-phase + household-assignment**. Project-queue persistence is the last remaining subsystem before the slice can close.

**Progress 2026-04-27.** Project-queue persistence audited and locked down by `BannerModSettlementProjectPersistenceTest` (14 cases covering both `PendingProject.toTag` / `fromTag` and `BannerModSettlementProjectScheduler.toTag` / `fromTag` / `restoreFromTag`). Leaf-record coverage: every `PendingProject` field survives a non-default-value roundtrip; the `NEW_BUILDING -> null target` ctor invariant is preserved across `toTag` so the on-disk representation matches in-memory state and `fromTag` doesn't accidentally rebuild a phantom UPGRADE binding; every `ProjectKind` and every `ProjectBlocker` enum value roundtrips to itself rather than silently falling back; an unknown future kind / blocker name does fall back (NEW_BUILDING / NONE) without crashing the loader. Aggregate-scheduler coverage: empty scheduler roundtrips to empty; multi-claim mixed-priority queues roundtrip in priority-sorted order so a regression that swapped to FIFO on load would surface; cancellation-log entries (every `ProjectCancellationReason` value, plus an unknown future reason that must fall back to MANUAL) survive the roundtrip; `PER_CLAIM_QUEUE_CAP` is enforced on the decode side so a hand-crafted save with overshoot entries is truncated, not honoured (regression guard against unbounded save bloat); identical `restoreFromTag` does not dirty (acceptance: "no false dirty churn on identical reload/restore") while a content-changing restore dirties exactly once; the decoded scheduler is mutable independently of the source (no shared internal collections); and `toTag` skips emptied claims so a polled-to-zero claim does not leak a stale queue tag into long-lived saves. Closes the last open subsystem for SETTLEMENT-004.

---

## SETTLEMENT-005 — Hauling and input-fetch work orders

**Status: DONE 2026-04-27.** `HarvestJobHandler` now claims FETCH_INPUT / HAUL_RESOURCE for floating-laborer residents (commit `3600d01`); `SettlementWorkOrderClaimReleaseEvents` returns claims to PENDING when a worker dies or is force-removed mid-route (commit `ec9e912`); live coverage from `BannerModWorkOrderTransportGameTests` and `BannerModWorkOrderClaimReleaseGameTests` plus the unit-tested `SettlementWorkOrderRuntime.releaseClaimsForResident`. All three acceptance bullets met.
...
**Progress 2026-04-27.** Both remaining gaps closed. (1) Floating-labor wiring (commit `3600d01`): `HarvestJobHandler.SUPPORTED_TYPES` now includes `FETCH_INPUT` and `HAUL_RESOURCE`, so floating-laborer residents bound to a building actually claim transport orders alongside gathering work — before this, `StockpileTransportWorkOrderPublisher` published the orders but no handler could pick them up, so they sat unclaimed forever. New `JobHandlerRegistryTest` cases lock the claim routing for both order types; new `BannerModWorkOrderTransportGameTests` exercises the live-registry runtime path end-to-end. Acceptance bullet "Workers/couriers can fetch inputs for production and haul outputs to storage" is now observable in-world. (2) Mid-route claim safety (commit `ec9e912`): `SettlementWorkOrderRuntime.releaseClaimsForResident(UUID)` returns the resident's active claim back to PENDING (idempotent on null / unknown residents); `SettlementWorkOrderClaimReleaseEvents` subscribes to `LivingDeathEvent` and `EntityLeaveLevelEvent` on `AbstractWorkerEntity`, with the leave handler gated on `RemovalReason.shouldDestroy()` so vanilla chunk-unload churn does not trip the release. Subscriber registered in `BannerModMain`. Live `BannerModWorkOrderClaimReleaseGameTests` covers both event paths. Acceptance bullet "Orders survive release/expiry correctly" is now observable for the death/removal case alongside the pre-existing reclaim-abandoned timer. Slice closed.
...
**Status: DONE 2026-04-27.** Pure-logic morale, rout-goal disengagement, shaken-hit dampening, and the suppression accumulator are all wired into the live recruit AI path; all three acceptance bullets are observable in-world.
...
**Progress 2026-04-27.** Combat-AI hookup landed for all three open follow-ups. (1) `RecruitMoraleService` is the per-recruit timeline driver: `tick()` runs from `RecruitRuntimeLoop.tick` once every `EVALUATION_INTERVAL_TICKS = 40` (2 s), pulls a `MoraleSnapshot` via `RecruitMoraleSampler`, and on a transition into `ROUTED` flips the recruit's `setFleeing(true)` flag and pushes a `bannermod.morale.routed` chat line to the owner naming the policy reason tokens — that's the "player sees why units routed" half of acceptance. (2) `RecruitMoraleRoutGoal` (priority 0 in `AbstractRecruitEntity#registerGoals`) consumes `RecruitMoraleService.isRouted` so a rout window of `ROUT_WINDOW_TICKS = 200` actually disengages the recruit: combat target cleared, `Flag.MOVE/LOOK/TARGET` taken, navigation walks `FLEE_STEP_DISTANCE = 8` blocks away from the nearest hostile every 10 ticks at `FLEE_SPEED = 1.25`. (3) Shaken-hit dampening: `MoralePolicy.attackMultiplierFor(MoraleState)` returns `SHAKEN_ATTACK_DAMPING_MULTIPLIER = 0.7` for SHAKEN and 1.0 otherwise; `RecruitCombatDecisions.doHurtTarget` chains it after the cavalry-charge multiplier so SHAKEN squads still swing but visibly hit softer. (4) Suppression accumulator: `RecruitMoraleService.recordDamageTaken(uuid, gameTime)` is called from `AbstractRecruitEntity#hurt` after `super.hurt` confirms damage actually applied (`amt > 0` + `applied == true` — blocked / shielded hits do not count). The accumulator runs a self-resetting sliding window of `SUPPRESSION_WINDOW_TICKS = 60` ticks; the sampler now reads `RecruitMoraleService.recentDamageEventCount(uuid, gameTime)` instead of capping at one event per `hurtTime` tick, so SUSTAINED_FIRE actually trips after enough hits land. Locked in by new `RecruitMoraleServiceTest` (8 cases — empty read, first event opens window at 1, six-event accumulation reaches the policy threshold, late event resets to 1 not 2, post-window read reports 0, invalidate clears, null UUID is a no-op, multiplier helper agrees with policy constant across STEADY/SHAKEN/ROUTED/null). Acceptance status: "badly outnumbered squads can rout" — observable (rout goal disengages on policy transition); "commander/nearby allies improve morale" — observable via aura-driven downgrade in policy + sampler; "player sees why units routed" — observable (chat line names the reason tokens). All three acceptance bullets met.

---

## COMBAT-002 — Officer/leader discipline aura

**Status: DONE 2026-04-27.** Aura policy is consumed by `RecruitMoraleSampler` to populate `MoraleSnapshot.commanderPresent`, which `MoralePolicy` uses to downgrade ROUTED -> SHAKEN at evaluation time — units near a same-faction commander measurably hold longer than isolated units, and the political-entity check in the policy makes the aura inert for enemy / neutral squads. Both acceptance bullets met by the COMBAT-001 hookup.
...
**Progress 2026-04-27.** Combat-AI wiring landed via `RecruitMoraleSampler`: when scanning the per-owner recruit index in the squad neighbourhood, every `AbstractLeaderEntity` ally is treated as a `CommanderAura` candidate and the snapshot's `commanderPresent` flag is set as soon as `CommanderAuraPolicy.isAuraActive` returns true. The political-entity check inside `CommanderAuraPolicy` is what enforces "aura does not buff enemies/neutral units" — leaders in the same Minecraft owner group still need the same political entity for the aura to project. Closes the slice; both acceptance bullets are met as a side effect of the COMBAT-001 hookup.
...
**Status: DONE 2026-04-27.** Cavalry-charge state machine, charge-multiplier on the recruit damage path, and pike brace flag are wired end-to-end; all three acceptance bullets observable from code (`CavalryChargeService.tick`, `RecruitCombatDecisions.doHurtTarget`, `UseShield`-driven `isBracing`).
...
**Progress 2026-04-27.** Combat-AI wiring landed (commit `12d865a`). `CavalryChargeService.tick` is invoked from `RecruitRuntimeLoop.tick` on every server-side recruit tick — it reads the recruit's vehicle delta-position to detect "currently galloping" and calls `CavalryChargePolicy.advance(...)` with `CHARGE_SPEED_THRESHOLD` as the gate. `RecruitCombatDecisions.doHurtTarget` chains `CavalryChargeService.computeChargeMultiplierFor(recruit, target)` onto the outgoing damage so the bonus / brace-penalty multiplier reaches non-recruit targets too, and on a successful hit `CavalryChargeService.onChargeHit` flips the state to EXHAUSTED via the policy, gating the next charge by `CHARGE_COOLDOWN_TICKS`. The `isBracing` flag on `AbstractRecruitEntity` is set from `UseShield` when a pike-class shieldman raises its shield against an oncoming rider, and is consumed by both the outbound multiplier (via the policy) and the inbound `RecruitCombatOverrideService.applyBraceAgainstCavalryMitigation` (cavalry attacker -> braced defender). Closes the slice; all three acceptance bullets observable in-world.

---

## COMBAT-005 — Ranged backline spacing and fallback

**Status: DONE 2026-04-27.** `RangedSpacingService` ticks each ranged recruit and caches a `RangedAction`; `RecruitRangedSpacingGoal` (registered for `BowmanEntity` and `CrossBowmanEntity`) consumes it to physically fall back when melee closes and lateral-shift to clear firing lanes. Both acceptance bullets observable in-world.
...
**Progress 2026-04-27.** Combat-AI wiring landed (commit `77db747`). `RangedSpacingService.tick` runs once per server-side recruit tick from `RecruitRuntimeLoop.tick`, samples the recruit's neighbourhood (nearest enemy melee distance, distance to own front line via the per-owner `RecruitIndex`, firing-lane occlusion against the recruit's current target), and caches the resolved `RangedAction` per recruit UUID. `RecruitRangedSpacingGoal` is registered at priority 1 in `BowmanEntity#registerGoals` and `CrossBowmanEntity#registerGoals` — above the regular ranged-attack and move-towards-target goals so a fallback decision takes precedence over closing in for a shot. The goal activates only when `currentAction` is FALLBACK or LATERAL_SHIFT (STAY yields, leaving the existing ranged combat goals running unchanged), then drives the recruit's navigation `FALLBACK_STEP = 4.0` blocks away from its target at speed 1.10 or `LATERAL_STEP = 3.0` blocks sideways at speed 1.00. Coexistence with the rout goal is explicit: rout sits at priority 0 with the same flag set, so a routed ranged recruit always defers to the rout window. Closes the slice; both acceptance bullets observable in-world.

---

## COMBAT-006 — Siege objective targeting and escort

**Status: DONE 2026-04-27.** `SiegeStandardBlock` damage path drains a per-record control pool through `SiegeObjectivePolicy.applyDamage`; `RecruitSiegeEscortGoal` pulls idle same-side recruits back inside the standard radius via `SiegeObjectivePolicy.shouldEscort`; destruction emits exactly one `SIEGE_STANDARD_DESTROYED` audit row. All three acceptance bullets observable in-world.
...
**Progress 2026-04-27.** Combat-AI / runtime wiring landed (commit `b84ef0f`). `SiegeStandardRecord` now carries `controlPool` + `maxControlPool` with NBT roundtrip; the existing 6-arg constructor stays as a save-compat bridge filling `DEFAULT_CONTROL_POOL`. `SiegeStandardRuntime.applyDamage` delegates to `SiegeObjectivePolicy.applyDamage` and replaces the record in place; `byPos(BlockPos)` is the new accessor used by the block-attack hook. `SiegeStandardBlock.attack` resolves attacker faction via `WarRuntimeContext.factionOf(player, registry)` (player -> politicalEntity through registry leader / co-leader matches), gates with `canAttackStandard`, drains 4 control per hit, writes a `SIEGE_STANDARD_HIT` audit row, and on destruction writes exactly one `SIEGE_STANDARD_DESTROYED` row before removing the runtime record and breaking the block. `RecruitSiegeEscortGoal` is registered at priority 7 on `AbstractRecruitEntity#registerGoals` — below explicit move/attack/protect goals so a player order still wins, but above wandering — and yields the recruit only when `recruit.getTarget() == null`, no `holdPos` / `movePos` is set, and `recruit.getFleeing()` is false. The goal walks the recruit toward the nearest same-side standard whose `shouldEscort` predicate matches, stopping `SLACK_BLOCKS = 4.0` short of the standard radius so escorts settle into the defended ring instead of clumping on the centre block. Closes the slice; all three acceptance bullets observable in-world.
